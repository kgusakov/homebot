...
impl<'a> TorrentFilter<'a> {
  ...
  pub fn new(telegram_client: &'a TelegramClient) -> Self {
    Self { telegram_client, transmission_client: TransmissionClient::new() }
  }

  pub fn process(&self, message: &Message) -> Result<(), Box<dyn std::error::Error>> {
    let process_success = |r: Response| {match r {
      Response { arguments: ResponseArguments::TorerntAdded {name: n, ..}, ..} => {
        self.telegram_client.send_message(message.chat.id, format!("{} успешно добавлен", n)).into()
      },
      Response { arguments: ResponseArguments::TorerntDuplicate {name: n, ..}, ..} => {
        self.telegram_client.send_message(message.chat.id, format!("{} уже был добавлен ранее", n)).into()
      },
    }};


    match message {
      Message {document: Some(doc), ..} if doc.file_name.ends_with(".torrent") => 
        self.process_torrent(&doc.file_id).and_then(process_success).map(|_| ()),
      _ => Ok(())
  }
}
  ...
  fn process_torrent(&self, file_id: &str) -> RequestResult<Response> {
    ...
    let content = self.telegram_client.donwload_file(&response.result.file_path)?;
...
impl TransmissionClient {
  ...
  fn new() -> Self {
    ...
    Self { transmission_address: transmission_address, http_client: reqwest::blocking::Client::new() }
  }

  fn req_with_sessions_id_loop<T: serde::de::DeserializeOwned>(&self, request: Request) -> RequestResult<T> {

    let resp: Result<reqwest::blocking::Response, Box<dyn std::error::Error>> = match self.http_client.post(&self.transmission_address).body(serde_json::to_string(&request)?).send() {
  ...
  pub fn torrent_add(&self, file_content: &[u8]) -> RequestResult<Response> {
    ...
    let request = Request {
      ...
      arguments: RequestArguments::TorrentAdd { filename: None, metainfo: Some(base64_encoded) }
    };
...
fn test_torrent_add_by_file_path() {
  env::set_var("TRANSMISSION_ADDRESS", "http://192.168.1.104:9091/transmission/rpc");
  let client = TransmissionClient::new();

  let request = Request {
    method: "torrent-add".to_string(),
    arguments: RequestArguments::TorrentAdd { filename: Some("/home/kgusakov/tt.torrent".to_string()), metainfo: None }
  };
