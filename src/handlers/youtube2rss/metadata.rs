...
impl Metadata {
  ...
  pub fn load_metadata<'a>(&self, s3_path: &'a str) -> Result<VecDeque<VideoMetadata>, Box<dyn std::error::Error>>  {
    Ok(match self.s3_storage.download_object(s3_path) {
      ...
      Err(e) => 
        match e.downcast_ref::<rusoto_core::RusotoError<rusoto_s3::GetObjectError>>() {
          Some(rusoto_core::RusotoError::Service(rusoto_s3::GetObjectError::NoSuchKey(_))) => Ok(VecDeque::new()),
          _ => Err(e)
      },
    }?)
  }

  pub fn update_metadata(&self, s3_path: &str, data: &VecDeque<VideoMetadata>) -> Result<(), Box<dyn std::error::Error>> {
    let mut buf = Vec::new();
