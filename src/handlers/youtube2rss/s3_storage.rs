...
impl S3Storage {
  ...
  pub fn download_object(&self, s3_path: &str) -> anyhow::Result<Vec<u8>>  {
    let response = self.s3_client.get_object(GetObjectRequest {
  ...
  pub fn upload_object(&self, data: Vec<u8>, s3_path: &str) -> Result<(), Box<dyn std::error::Error>> {
  ...
  pub fn upload_file(&self, file: &Path, s3_path: &str) -> Result<(), Box<dyn std::error::Error>> {
  ...
  pub fn get_public_url(&self, s3_path: &str) -> String {
    format!("https://storage.yandexcloud.net/{bucket}/{file}", bucket=self.bucket_name, file=s3_path)
  }
}